...
갭 체결(지정가 아래로 갭 나면 그날 시가에 채워진다)까지 쳐 준 뒤에도 OOS 에서
가장 좋은 실행이 **−0.072R** (p=0.98) 이다. 비용 전으로도 +0.071R — 0 이다.

**아래 코드는 그대로 쓸 수 있다** — 막힌 것은 진입 규칙이지 주문 처리가 아니다.

⛔ **2026-08-13: 라인 하나로 좁혀 다시 쟀다. 양수가 없다.** 안 잰 라인 둘을
채웠다 — 구간 하단 지정가(D)와 되돌림을 안 기다리는 시장가(E). 걸 수 있는 것
중 가장 나은 E 가 OOS **−0.036R** (p=0.998, 체결률 90%) 이고, **비용 전으로도
+0.022R** 이다. 손절폭이 중앙 1.38% 로 넓어 6bp 가 0.044R 밖에 안 먹는데도
남는 게 없다 — 수수료 문제가 아니다.

그래서 **유령 수익의 정체가 갈렸다: 셋업 선택이 아니라 싸게 산 값이었다.**
역선택이 원인이면 되돌림을 안 기다리는 E 가 그 판들을 되찾아야 하는데, E 는
유령이 잡던 셋업을 90% 다 잡고도 0 이다. **다음에 손볼 것은 주문 방식이 아니라
신호다.** 진입 실행은 더 짜 볼 것이 없다.
...
def main() -> int:
    ...
    if os.environ.get("RUN_KNOWN_NEGATIVE") != "true":
        print("정지 — 이 규칙은 실행 가능한 형태로는 기대값이 음수입니다.\n"
              ...
              "  실제로 걸 수 있는 최선이 OOS −0.072R (p=0.98) 입니다.\n"
