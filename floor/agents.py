...
ATTACK = Mode(
    ...
    # 판정이 아니라 연출이라는 걸 화면에 그대로 띄운다. 다만 연출이라고 해서
    # 검증 관문을 면제하지는 않는다 — 방향은 고르되 그 매매가 가능한지는 엔진이
    # 15분봉으로 다시 잰다. 면제되는 거부권은 거부권이 아니다.
    note="관망 금지 — 반드시 방향을 고른다. 연출이며 시연·영상용. 관문은 그대로 걸린다.",
)
