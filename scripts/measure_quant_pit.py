...
def main() -> int:
    ...
    body += [
        ...
        "",
        f"②의 95% 구간이 [{lo:+.2f}, {hi:+.2f}] 로 넓다. {len(days)}일 · 자리 "
        f"{MAX_POSITIONS} · 실현 곡선이라 이 검정의 힘이 약하다는 뜻이고, 읽을 수 있는 "
        "것은 **\"총괄 판정을 붙이면 나빠진다\"가 아니라 \"나아진다고 말할 근거가 "
        "없다\"** 뿐이다. 위약 줄의 구간도 거의 같은 폭이라 그 판단은 점추정의 부호와",
