...
**Two layers of testing exist.** (1) A **pytest suite** (`tests/`, ~680 tests across `test_bulls_signals`, `test_edgar_fundamentals`, `test_factor_formulas`, `test_factor_scores`, `test_factor_timing`, `test_ic_weights`, `test_price_panel`, `test_sectoral_scores`, `test_system_signals`, `test_universe`, `test_smoke`) runs fast and network-free; `ci.yml` gates every push to `main` and every PR with `ruff check .` + `pytest tests/`. (2) **Statistical validation** of the strategy lives in `modules/` (`factor_validator.py`, `stat_validation.py`, `survivorship_check.py`, `stress_test.py`) and is surfaced through the app's 퀀트 → 고급 분석 / 운영 안전성 sub-tabs. `ic_weight_updater.py` is the headless entry point that drives `factor_validator` end-to-end. Add a pytest test when you touch pure logic in `modules/`; keep them network-free so CI stays green.
...
- `modules/factor_engine.py` **no longer scores anything.** It used to hold a standalone 5-factor scanner (`calc_factor_scores`/`generate_signals`/`fetch_fundamentals`/`fetch_returns_matrix`); the paper-trade runner moved to trade plans on 2026-08-10 and nothing called them after that, so they were deleted on 2026-08-19 (491 → 167 lines). What remains has exactly three consumers: `get_market_regime` (runner + `scripts/`), `point_in_time_fundamentals` (`factor_validator`'s walk-forward IC), and `REGIME_WEIGHTS` (`ic_weight_updater`'s baseline).

So there is now **one** scoring engine — `app.py` + `modules/factor_scoring.py`. The IC pipeline still measures its own factor definitions (below), but no second scanner ranks stocks in parallel any more.
...
**Price factor formulas now live in `modules/factor_formulas.py`** alongside the value/quality blends — `momentum_pct(close, lookback_bars, skip_bars=0)` and `annualized_vol_pct(close, window_bars)`. The three remaining call sites (`factor_scoring.price_factors` and both of `factor_validator`'s scoring passes) call them **each passing its own window**, so the divergence is visible as arguments instead of hidden in near-identical expressions. (`factor_engine._momentum` was a fourth until its scanner was deleted on 2026-08-19.) Deliberately behavior-preserving: the windows were left exactly as they were, including a one-bar mismatch *inside* `factor_validator` (`_calc_momentum_vol_scores` uses 63/21, `_calc_per_factor_zscores` uses 64/22). `tests/test_factor_formulas.py` pins every window in the `LEGACY_*_CALLSITES` tables and fails CI if an inline copy reappears. Those tables shrinking to one row is the definition of done for this work.
...
`app.py` imports every module inside `try/except`, setting `_*_AVAILABLE` flags (e.g. `_ML_AVAILABLE`, `_OPS_SAFETY_AVAILABLE`). The app degrades gracefully when a module or its dependency is missing, so features are guarded by these flags. Key modules: `factor_engine`, `factor_validator`, `ict_analysis`, `ml_signals`, `risk_management`, `ops_safety` (kill switch / reconciliation), `alpaca_trading` + `virtual_broker` (current broker path). Point-in-time fundamentals discipline lives in `factor_engine.point_in_time_fundamentals` (EDGAR `filed`-indexed panel), not a separate DB.

### The weekly IC feedback loop

`REGIME_WEIGHTS` in `factor_engine.py` are only a weak prior. Every Sunday `ic_weight_updater.py` computes 5-year walk-forward Information Coefficients and writes `ic_weights.json`; the live scan reads `production_weights` from that file to rescale factor weights (low/negative-IC factors shrink automatically, floored at `IC_FLOOR`).

**`regime_weights` in that file currently has no reader.** Its only consumer was `factor_engine._load_regime_weights`, deleted with the dead scanner on 2026-08-19. The job still computes and stores the block (`REGIME_WEIGHTS` remains its baseline); decide whether to keep producing it before adding anything that depends on it.
