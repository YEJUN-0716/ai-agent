...
2. 국가는 **대한민국**, 홈 리전은 아래 우선순위로 선택
   - 홈 리전은 **나중에 못 바꿉니다.** 다만 **어느 리전이든 동작합니다** — 이 서버에 필요한 건 고정 IP와 24시간 가동뿐이고, 하루 한 번 리포트를 보내는 용도라 지연시간 차이는 무의미합니다.

   | 순위 | 리전 |
   |---|---|
   | 1 | South Korea Central (서울) / South Korea North (춘천) |
   | 2 | Japan East (도쿄) / Japan Central (오사카) |
   | 3 | Singapore |

   > 목록에 서울이 없을 수 있습니다. 가입 폼의 **국가가 대한민국인지** 먼저 확인하고(홈 리전 목록이 국가로 걸러집니다), 그래도 없으면 다음 순위로 가면 됩니다.
   >
   > 무료 ARM 서버(A1)는 한국 리전이 자주 포화입니다. 일본 리전이 오히려 재고가 있는 경우가 많습니다.
