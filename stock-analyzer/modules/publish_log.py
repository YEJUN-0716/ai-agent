...
def last_published_n(horizon, root=LOG_DIRNAME):
    """그 지평에서 **지금까지 발행한 것 중 가장 큰** 표본 수. 없으면 None.

    "마지막"이 아니라 max() 인 것은 의도다 — 이 값은 new_horizons() 의
    "표본이 늘었나" 게이트에만 쓰인다. 기록이 지워지거나 백필로 n 이
    한 번 줄어든 뒤 마지막 값을 기준 삼으면 같은 판정을 다시 발행한다.
