...
> **실행 중 변경 — 손상 파일 처리 (사장님 판단, 2026-07-29).**
> 아래 Step 3의 `_load_list`는 망가진 파일을 만나면 빈 목록으로 새로
> 시작한다. 그러면 다음 쓰기가 그 파일을 덮어써서 기존 기록이 영구히
> 사라진다는 점이 리뷰에서 드러났고, 사장님이 **원본 보관**을 택했다.
>
> 확정된 요구사항:
> - **존재하는** 파일을 파싱하지 못했을 때(또는 리스트가 아닐 때), 비우기
>   전에 원본을 같은 폴더에 `<파일이름>.<KST타임스탬프>.corrupt`로 옮긴다.
>   반복 실패해도 이름이 겹치지 않아야 한다.
> - 옮긴 사실을 감사 기록에 남긴다 (action `file_corrupt`, detail에 원본과
>   백업 이름).
> - 그 뒤 빈 목록을 반환해 비서는 계속 동작한다.
> - 파일이 **없는** 것은 손상이 아니다 — 백업도 감사 기록도 남기지 않는다.
>
> 내부 구현 형태(헬퍼 분리 여부, 시그니처)는 구현자가 정한다. 아래 코드
> 블록은 이 변경 이전의 원문이며, 이 주석이 우선한다.
