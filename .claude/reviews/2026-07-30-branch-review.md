...
**결정**: ~~BLOCK~~ → **해소됨** (2026-07-30, 커밋 `ac74028`·`48d9d4f`)

## 처리 결과

| 항목 | 상태 | 커밋 |
|---|---|---|
| C1 동시 승인 중복 주문 | 고침 — 대기목록 읽기-수정-쓰기를 잠금으로 감쌈 | `ac74028` |
| H1 감사 로그 위조 | 고침 — 개행·구분자 이스케이프, 필드 길이 상한 | `ac74028` |
| M1 `ASSISTANT_WEB_HOST` 고장 | 고침 — 루프백이 아니면 시작 시 `ConfigError` | `48d9d4f` |
| M2 비원자적 쓰기 | 고침 — 임시파일+이름바꾸기(`save_json_list`)로 통일 | `48d9d4f` |
| M3 `sys.path` 오염 | 고침 — 맨 앞이 아니라 뒤에 추가 | `48d9d4f` |
| M4 도구 루프 상한 없음 | 고침 — `MAX_TOOL_ITERATIONS`에서 끊고 알림 | `48d9d4f` |
| M5 장애 로그 미출력 | 고침 — 감시 코루틴으로 죽는 즉시 기록 | `48d9d4f` |
| L2 `UnicodeDecodeError` 미포착 | 고침 — `StockDataError`로 변환 | `48d9d4f` |
| L3 `text[:60]` 매직넘버 | 고침 — `MAX_AUDIT_FIELD` 상수 | `ac74028` |
| L4 감사 로그 파싱 가정 | 고침 — H1과 동일 근원 | `ac74028` |
| L1 예외 메시지 노출 | 보류 — 현 라이브러리는 키를 예외에 담지 않음 | — |
| L5 기존 파일 커버리지 0% | 보류 — 이번 작업 범위 밖 | — |

C1·H1·M1·M4는 고친 코드를 되돌렸을 때 테스트가 실제로 실패(M4는 무한루프
시간초과)하는 것을 확인했다. 최종 테스트 97개 통과.

---

**최초 검토 시 결정**: **BLOCK** — CRITICAL 1건
