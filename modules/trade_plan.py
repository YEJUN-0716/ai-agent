...
# 같았다**. 다른 건 비용 내성뿐이었다 — 경계는 그때 정한 그대로다.
#
# **2026-08-16 숫자 교체.** 옛 손익분기(88.8/53.3/41.7/29.6bp)는 걸 수 없는
# 체결가로 잰 유령 판이었다. 걸 수 있는 지정가로 다시 재면 롱 기준:
#
#     A 손절 >2.34%   손익분기 편도 36.2bp   (n=2,529)
#     B     >1.75%              18.5bp   (n=2,504)
#     C     >1.34%              14.9bp   (n=2,603)
#     D    <=1.34%              13.2bp   (n=2,646)  ← 실제 비용 구간 안
#
# 롱 기준이다 — 숏은 비용 0 에서 이미 음수라 등급으로 구할 수 있는 게 아니고,
# 실행 문턱(ACTIONABLE_DIRECTIONS)이 이미 뺀다. 섞어서 재면 A 아래 세 등급이
# 9.5~10.0bp 로 붙어 사다리가 사라진다.
#
# 결과를 보고 고르는 게 아니라 기하를 보고 고르는 등급이라, 흔한 백테스트
# 필터보다 데이터마이닝 위험이 낮다. 경계를 옮기려면 측정을 다시 할 것
# (`python scripts/measure_trade_cost_sensitivity.py`).
COST_GRADE_BANDS = ((2.34, "A"), (1.75, "B"), (1.34, "C"))
COST_GRADE_BREAKEVEN_BP = {"A": 36.2, "B": 18.5, "C": 14.9, "D": 13.2}
