...
# 국내(KRX) 상장주식 세금
# ─────────────────────────────────────────────
# 소액주주의 국내 상장주식 양도차익은 현행 비과세(대주주 요건 미충족 시).
# 금융투자소득세는 2025→2027로 시행 유예. 따라서 소액주주가 실제로 내는 세금은
# '매도 시 증권거래세'가 사실상 전부다. 세율은 연도별로 변동하므로 파라미터화한다.

# 2024년 매도 시 증권거래세율(농어촌특별세 포함, %). 연도가 바뀌면 갱신 필요.
DOMESTIC_TX_TAX_RATE = {
    'KOSPI':  0.18,   # 증권거래세 0.03% + 농특세 0.15%
    'KOSDAQ': 0.18,   # 증권거래세 0.18% (농특세 없음)
    'KONEX':  0.10,
}


def calc_domestic_transaction_tax(sell_proceeds_krw: float, market: str = 'KOSPI',
                                   rate_pct: float = None) -> dict:
    """
    국내 상장주식 매도 시 증권거래세(농특세 포함) 계산.

    sell_proceeds_krw : 매도 대금(원)
    market            : 'KOSPI' | 'KOSDAQ' | 'KONEX'
    rate_pct          : 세율(%) 직접 지정 시 우선. 미지정이면 시장별 2024년 기본값.
    """
    mk = (market or 'KOSPI').upper()
    rate = rate_pct if rate_pct is not None else DOMESTIC_TX_TAX_RATE.get(mk, 0.18)
    tax = round(max(0.0, sell_proceeds_krw) * rate / 100)
    return {
        'market': mk,
        'sell_proceeds_krw': round(sell_proceeds_krw),
        'tax_rate_pct': rate,
        'transaction_tax_krw': tax,
        'note': (f"{mk} 매도대금 {sell_proceeds_krw:,.0f}원 × {rate}% = 증권거래세 "
                 f"{tax:,}원(농특세 포함, 매수 시엔 미부과). 소액주주 양도차익은 현행 "
                 f"비과세(금융투자소득세 2027년까지 유예). 세무사 최종 확인 필수."),
    }


def estimate_krx_annual_tax(sell_trades: list) -> dict:
    """
    국내 매도 내역 리스트로 연간 증권거래세 합계 추정.

    sell_trades: [{'sell_proceeds_krw': float, 'market': 'KOSPI'|'KOSDAQ'|...}, ...]
    """
    items = [calc_domestic_transaction_tax(t.get('sell_proceeds_krw', 0),
                                           t.get('market', 'KOSPI'))
             for t in sell_trades]
    total = sum(it['transaction_tax_krw'] for it in items)
    return {
        'n_sells': len(items),
        'total_transaction_tax_krw': round(total),
        'breakdown': items,
        'note': "국내 소액주주는 양도소득세 없음 — 매도 증권거래세 합계만 추정. "
                "대주주(종목당 지분·평가액 요건 충족) 해당 시 별도 양도세 발생.",
    }
