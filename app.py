        ...
        if 'tab1' not in st.session_state:
            ...
            with cv:
                ...
                <div style='text-align:center;padding:20px 0'>
                  ...
                  <br><div style='color:#c0c0c0;font-size:15px'>
                  차트+파동 <b>{t_score:.0f}</b>점 &nbsp;|&nbsp;
                  재무+퀀트 <b>{f_score:.0f}</b>점 &nbsp;|&nbsp;
                  매크로+금리 <b>{m_score:.0f}</b>점</div>
                  <div style='color:#b0b0b0;font-size:11px;margin-top:6px'>
                  📐 {score_method}</div>
                ...
                if trade_signals:
                    ...
                    for sig_i, (sig_ico, sig_nm, sig_dc) in enumerate(trade_signals):
                        ...
                        sig_cols[sig_i % sig_n].markdown(
                            ...
                            f"<span style='color:#c0c0c0;font-size:12px'>{sig_dc}</span></div>",
                            unsafe_allow_html=True)
            ...
            with sub4:
                ...
                def _render_strategy(label, icon, color, bg, tr, cp_ref):
                    ...
                    st.markdown(
                        ...
                        f"<div style='color:#ccc;font-size:13px;margin-top:4px'>{tr['strategy']}</div>"
                    ...
                    st.markdown(f"<div style='font-size:12px;color:#c0c0c0'>{cond_text}</div>",
                                unsafe_allow_html=True)
                ...
                for plan_col, plan_key in zip(plan_cols, ['dantta', 'swing']):
                    ...
                    with plan_col:
                        st.markdown(
                            ...
                            f"<div style='color:#c0c0c0;font-size:12px;margin-top:6px'>{notes_text}</div>"
                            f"</div>", unsafe_allow_html=True)
            ...
            with sub5:
                ...
                with ns_col1:
                    ...
                    st.markdown(
                        ...
                        f"<div style='color:#c0c0c0;font-size:11px;margin-top:4px'>감성 점수</div>"
                        f"</div>", unsafe_allow_html=True)
                with ns_col2:
                    if news_articles:
            ...
            if 'qt_factors' in st.session_state:
                ...
                for _, r in top5.iterrows():
                    ...
                    st.markdown(
                        ...
                        f"<span style='color:#c0c0c0'>{r['name']}</span></span>"
            ...
            if 'qt_signals' in st.session_state:
                ...
                for a in sorted(actions, key=lambda x: {'HIGH':0,'NORMAL':1,'LOW':2}.get(x['priority'],3)):
                    ...
                    st.markdown(
                        ...
                        f" <span style='color:#b0b0b0;font-size:11px'>{a['reason']}</span></span>"
                        f"<span style='color:#c0c0c0;font-size:12px'>목표 {a['weight']}"
