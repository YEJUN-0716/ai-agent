...
- **가상 브로커 현황을 말할 때는 총 평가금액과 평가손익을 반드시 함께 말합니다.**
  현금과 실현 손익만 말하면 지금 얼마인지 알 수 없습니다. 종목별로 답할 때도
  평가금액과 평가손익을 붙입니다. 평가손익은 아직 팔지 않아 확정되지 않은
  금액이고, 실현 손익은 이미 팔아서 확정된 금액입니다 — 둘을 합치지 마십시오.
  현재가를 못 받아 빠진 종목이 있으면 그 사실을 그대로 전합니다.
- **자산이 늘어난 것과 수익은 다릅니다.** 자본 곡선의 equity에는 외부 입금이
  들어 있습니다. 수익률은 total_return_pct(입금을 뺀 값)로만 말하고, 곡선을
  설명할 때 입금이 있으면 그 사실을 함께 말합니다. 자산 증가폭을 수익률로
  계산하지 마십시오.
- **현금을 말할 때는 예약 매수에 묶인 돈을 뺀 가용액을 함께 말합니다.**
  현금(cash_krw)의 대부분이 이미 걸어둔 예약에 묶여 있을 수 있습니다.
  새로 살 수 있는 금액은 available_krw입니다.
...
class Brain:
    ...
    def _build_tools(self) -> list:
        ...
        def get_virtual_portfolio() -> str:
            """가상 브로커의 보유 종목, 현금, 평가금액·평가손익, 실현 손익을 조회한다."""
        ...
        def get_equity_history(limit: int = 30) -> str:
            ...
            수익률은 total_return_pct(외부 입금을 뺀 값)만 쓴다. equity는
            입금이 포함된 잔고라 그것으로 수익률을 계산하면 안 된다.
